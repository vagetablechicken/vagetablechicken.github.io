...
题目说明了，k个list，list内的区间都是有序的。这很像k路归并排序。按理可以比所有inteval混在一起排序更高效。那我们先mark这个排序，放在一边。

假设现在已经得到了所有interval的排序结果，求反只需要遍历就行了。这种思路的优化就是不要全排序，而利用list内interval有序来做类似归并排序的操作。

还可以想到，我先不把所有list合在一起，我每个list先求反，得到每个员工的空闲时间，所有的空闲时间的交集就是最终答案。但空闲时间区间个数不见得比工作区间个数少多少（应该是，对每个list，空闲区间=工作区间-1）。并没有把复杂降低的感觉。而且空闲时间的交集这个概念比并集复杂，因为空闲区间没办法全部一起求交集。想象一下，假设5个员工，有1个在[0,t]之间都不空闲，但是另外4个可以求出交集，但是这个交集是不能要的，因为有一个人不空闲。区间全混在一起时，根本无法判断。所以还是求并集吧。

回归“求并集再求反”的思路。全排序的方法很好写。不过我老老实实先求并再求反，实际上这两步可以并成一步。“有序区间列表”的并集再求反，其实就是求区间的gap。当所有工作区间排序好后，上一个和下一个区间如果有重合，就说明没有gap。没有重合时，这里一定有一个gap。所以一次遍历就可以了。

当然，还是避不开归并优化的问题。

#### 归并思路

归并思路，k个list也就是k路，每次都从k个list的头上选出最小的。这个pick出来的区间，和前面pick出来的区间是有序的，此题并不是要最终排序的列表，所以没必要append to list，做相应的处理就行了。后面处理部分先放着，核心是要考虑“k个中选最小的”怎么高效，基本堆算法就两路，if-else就行了，k路肯定是不可能一个一个比的，全排序也是不可能的，很明显可以利用最小堆，每次的top就是最小的，top取出来后，top所在的list就应该补充一个区间进入最小堆。python中也就是使用heapq，可以极快的写完代码。

但到这里，还可以进一步优化，这个也需要理解，因为k路归并的常见算法是败者树。败者树也是考点之一。

##### 败者树

首先思考堆有什么不尽人意的地方？pop堆顶，为了保证堆，是得把last one放到[0]并往下沉的，然后你又push一个数进来，又得把数append到数组尾部并往上浮。

- [ ] python这里pop是调用_siftup，啥情况？

需要注意的是，就算改进堆，也不可能有数量级的变化，单步复杂度logk是不可能直接到常数复杂度O(1)的，排序能到O(1)就不用学这么多树的算法了。

但确实，普通的堆，这里要做两次操作，而且没办法将两次操作合并。所以在堆结构上不用继续下去了，需要一个新的数据结构。
...
为什么这么做呢？首先是胜者树的重新比赛，是叶子和它的临近节点比，得去找临近节点。而败者树，只需要和它的父节点比。重点就是理解“为什么败者树节省了步骤”。

这里粗略一想，觉得很莫名，假设两个临近节点n1、n2，n1更小，n1是胜者，n2就是父节点，我如果修改n2，不还是得跟n1去比？

大错特错🐶

注意胜者树和败者树的属性，只有胜者会被选出去，然后加入一个新元素，这个新元素初始的位置应该是原来胜者的位置。败者树也是同样的，会把胜者拖出去，胜者的那个位置被改成新元素，然后从新元素这个节点开始，去调整树。不会搞什么随意选一个节点修改。

所以发现了败者树的特点了吗？整棵树的胜者在“从它往上直到根节点”的这一路上，它都是胜者。如果我们走胜者树，当我们踢走胜者，这一路上的节点都没有了作用，导致我们必须去找临近节点才能比出个结果。而败者树，我们总是记录了被全局胜者pk掉的败者，其实也就是全局胜者每一层的临近节点，我们就不用去找了，直接向上一路比较就可以了。

逻辑上走通了，再来说说code的问题。两个叶子节点比较，得到败者作为父节点，这里还很常规，但是还得往上送胜者，这里就微妙了。我们应该怎么做？

首先整理下现有条件。k路是确定的，那么我们“可以考虑”，创建，1个k长数组，装真实元素，一个得算一下才知道多长的数组来装败者树，但这样的代价是，败者树里只有败者信息，每次比较要用的胜者得另外放。假设败者树已经构建好，那么这个胜者的临时信息还好说，只需要一个变量就可以搞定。但构建败者树时，就不够用了。n个叶子节点，那么第一次就需要(n+1)/2个变量来存胜者idx。然后依次往上，一层一层构建。还得考虑下某一层为奇数个节点时，最右边的链路可能要特别处理。

这让人不得不考虑，这里是不是用node指针来做，可以减少各种微妙的边界考虑。

其实这就是败者树那个奇怪的头节点的意义。败者树是一个“全局胜者“节点下面挂一颗完整二叉树（全是败者idx）。

规模最小化，只有一个元素时，这个小树就是一个胜者节点，下面挂null（暂定）。首先构造出这么k颗树，放在list里，然后两两合并，每个树的顶点都是其胜者，胜者和胜者比，选出败者，这个新败者要作为父节点，但注意这个新败者的子节点不是两个胜者，而是两个胜者下面挂的树顶点（根据规则，胜者节点下面只会有一个节点，也就是完整二叉树的顶点，不会有两个）。这时候，新败者可以直接改指针得到，而新胜者也不用创建，指针链接改改就行。就可以做成”新胜者下面挂新败者，新败者有两个子节点“。

但注意，前面暂定的是挂null，那么就导致两个最小树合并后，下面那个完整二叉树只有一个败者节点。举例说明，两个最小树是，1->两个null, 2->两个null，假设元素2胜出，合并后就是2->1->两个null。这并不能构造出概念上的败者树，想一想也知道，当我们把元素2踢出，都不知道怎么从下往上调整败者树。所以最小树的设计是不对的。最小树的设计应该是2->2->两个null。第一个2是胜者节点，下面挂的是完整二叉树，这里理解上会很奇怪，“完整二叉树里元素2是败者，但它也是整个树里的胜者”很诡异。所以，应该定义叶子节点是真实元素，并非胜者败者。那么这里就出现了三种节点定义，胜者节点，败者节点（一起组成完整二叉树），叶子节点。

再考虑剔除胜者节点指向的元素时，得立马能找到这个叶子节点，所以叶子节点的地址应该用一个数组保存，比如idx 2的元素为全局胜者，它被更新了，我们可以直接[2]拿到叶子节点，它本身不变还是指的元素2，可以记为l_idx=2，但是它得访问它的parent，得到parent指向的idx，记为p_idx，然后arr[l_idx]和arr[p_idx]比一下，然后一路往上。所以，节点们还必须有parent这个成员。

再整理一遍数据结构，首先给定k个元素（可以理解为一个k长的数组），我们对每一个元素构造一个最小败者树，最小败者树一定是“一个胜者节点挂一个叶子节点”（叶子节点不可省略，它的value是数组的idx，左右指针都为null）。这k个树的胜者节点得缓存一下。然后把缓存的每两个拖出来合并，再把新的树缓存到另一个地方。不断进行合并，直到缓存区只有一个胜者节点。用这个胜者节点就能拿到完整的败者树。

败者树pop又push的操作，具体算法又该是什么样呢？
