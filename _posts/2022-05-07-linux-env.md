...
ShellCrash 需要更改配置文件添加规则，面板只能查看。通过`6-导入配置文件`(是的，很费解，但就是走这儿)，然后可以新增删除规则。批量操作有提示文件地址在哪里，例如`/etc/ShellCrash/yamls/rules.yaml`，可以直接编辑这个文件。可以`5-自定义高级功能`让它把所有配置文件都生成。

目前能把tailscale网络恢复的方法，主要用于git clone等操作：

- ShellCrash 中 6 - 2 添加自定义规则，或者直接编辑 `/etc/ShellCrash/yamls/rules.yaml`
- 添加tailscale网段，`- IP-CIDR,100.64.0.0/10,DIRECT,no-resolve`
- 重启ShellCrash
- git项目使用ip而不要用域名，例如`git clone http://100.64.x.x/xxx.git`

配置上比较简单的可以使用 [clash-for-linux-install](https://github.com/nelvko/clash-for-linux-install)，它可以一次性安装并配置好订阅地址，以及ui界面，更适合不想折腾的用户。之后补规则，`clashmixin -e`直接在配置文件中配置。

订阅解析可能不通（主要还是订阅地址的问题），要用用户级软件转出yaml配置，再写到reousrces目录的config.yaml里，再install。

```bash
# 只关代理环境变量，不关代理进程，避免流量全走代理了
clashproxy off

clashproxy  # 检查状态

clashmixin -e  # 编辑配置文件，添加规则
```

常用检测地址：
```bash
# 如果地区限制，会有明显提示，gemini优先返回key，看不到区域
curl https://api.openai.com/v1/models
# 如果代理成功，会有先connection established的提示，然后再是openai认为无权限authentication等错误
curl https://api.openai.com/v1/models -x http://127.0.0.1:7890
```
