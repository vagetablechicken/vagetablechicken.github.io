...
TopK，面试时也遇到过，我认为heap K最稳定，但面试官更希望我用快排思想，虽然快排思想有一定几率会很慢--最坏O(n^2)。除了复杂度，实际运行，由于现代计算机架构，inner loop可以很快，见[wiki](https://zh.wikipedia.org/zh-cn/%E5%BF%AB%E9%80%9F%E6%8E%92%E5%BA%8F)。具体的复杂度算法也可以看wiki。不要仅考虑理论复杂度。

不过有个点，我也不知道是不是记错了，面试官当时可能说c++ stl的paritial_sort是快排思想，但实际paritial_sort就是先构建k大小的堆，然后把剩下的都和堆顶比。nth_element才是快排的思路。

核心快排思路见 [Quick Sort](./2022-10-07-abs-algo.md#quick-sort)。
